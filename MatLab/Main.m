...
%             THE TALK BOX EFFECT             % 
%                                             %
%   Anna Chiara Meioli & Gianluigi Vecchini   %
%                                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:nFrames
    ...
    % OLA
    y(idxFrame) = y(idxFrame) + (yFrame.* w)';
    e(idxFrame) = e(idxFrame) + (e_x.* w)';
    ...
    if i == 50
        ...
        plot(linspace(0, Fs/2, Nfft/2+1), S - max(S), 'k'); hold on;
        plot(linspace(0, Fs/2, Nfft/2+1), E_X - max(E_X), 'b'); hold on;
        plot(w_s, Y - max(Y), 'm');
